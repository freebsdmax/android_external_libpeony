...
static void __hexdump(char *name, const void *_data, size_t size)
{
...
#ifdef LIBPEONY_ENABLE
    { "video/libpeony", "OMX.google.libpeony.decoder.video" },
    { "audio/libpeony", "OMX.google.libpeony.decoder.audio" },
#endif
...
uuprintf("______-------OMXCodec::findMatchingCodecs %d, %s, %x, %x", index, componentName, flags, kPreferSoftwareCodecs);
...
sp<MediaSource> OMXCodec::Create(
    ...
    Vector<String8> matchingCodecs;
uuprintf("______-------OMXCodec::Create===findMatchingCodecs %s, %s", mime, matchComponentName);
/*
// test start
        int32_t width;
        meta->findInt32(kKeyWidth, &width);

        if( width < 360 ) findMatchingCodecs(mime, createEncoder, matchComponentName, flags | 0x01, &matchingCodecs);
    else
// test end..     
*/   
...
uuprintf("______-------OMXCodec::Create===findMatchingCodecs not found");
        return NULL;
    }
...
uuprintf("______-------OMXCodec::Create===finded %d, %s", i, matchingCodecs[i].string());
        ...
        if (err == OK) {
uuprintf("______-------OMXCodec::Create===done %s", componentName);
...
uuprintf("========== OMXcodex ========== %d:%d", mFlags, kIgnoreCodecSpecificData);
...
//uuprintf("========== OMXcodex ========== %d:%d", type, size);
//__hexdump("kkv-OMXcodex-kvk", data, size);
...
uuprintf("_____-----_____----- setVideoOutputFormat 00 %d", success);
            CHECK(success);
uuprintf("_____-----_____----- setVideoOutputFormat 11 %d %d", width, height);
            status_t err = setVideoOutputFormat(mMIME, width, height);
uuprintf("_____-----_____----- setVideoOutputFormat 22 %d", err);
...
uuprintf("_____-----_____----- setVideoPortFormatType 11 = %d, %d, %d, %d",
        format.eCompressionFormat, compressionFormat, format.eColorFormat, colorFormat);
...
uuprintf("_____-----_____----- setVideoPortFormatType 22 = %d, %d",
        index, kMaxColorFormatSupported);
...
void OMXCodec::setVideoInputFormat(
    ...
    //////////////////////// Input port /////////////////////////
uuprintf("_____-----_____----- call setVideoPortFormatType = %d, %d, %d", kPortIndexInput, OMX_VIDEO_CodingUnused, colorFormat);
    ...
    //////////////////////// Output port /////////////////////////
uuprintf("_____-----_____----- call setVideoPortFormatType = %d, %d, %d", kPortIndexInput, compressionFormat, OMX_COLOR_FormatUnused);
    CHECK_EQ(setVideoPortFormatType(
...
uuprintf("_____-----_____----- setVideoOutputFormat 00 %d, %d", kPortIndexInput, compressionFormat);
uuprintf("_____-----_____----- call setVideoPortFormatType = %d, %d, %d", kPortIndexInput, compressionFormat, OMX_COLOR_FormatUnused);
    status_t err = setVideoPortFormatType(
            kPortIndexInput, compressionFormat, OMX_COLOR_FormatUnused);
uuprintf("_____-----_____----- setVideoOutputFormat 11 %d", err);
...
#if defined(BOARD_USES_HDMI) && (defined(SAMSUNG_EXYNOS4x12) || defined(SAMSUNG_EXYNOS5250))
