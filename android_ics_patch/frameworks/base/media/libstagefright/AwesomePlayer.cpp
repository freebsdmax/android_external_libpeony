...
status_t AwesomePlayer::setDataSource(
    ...
    if (libHandle == NULL) { LOGE("unable to dlopen libpeony_demuxer.so"); return UNKNOWN_ERROR; }
    ...
    if (create_DataSource_func == NULL) { LOGE("unable to dlsym create_DataSource"); dlclose(libHandle); libHandle = NULL; return UNKNOWN_ERROR; }
