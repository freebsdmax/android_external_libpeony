...
#define LOG_TAG "__libpeony_b2_connect__"
#include <utils/Log.h>
...
__FORMAT_MATCH__        __format_match__[] =
{
    ...
    {__ITY_MPEGPS__,          __FORMAT_AV_TY_AV__,    1,	"mpeg"},
    {__ITY_MPEGTS__,          __FORMAT_AV_TY_AV__,    1,	"mpegts"},
#ifdef __DP_FFMPEG_NEW__
    {__ITY_MKV__,             __FORMAT_AV_TY_AV__,    1,	"matroska,webm"},
#endif
#ifdef __DP_FFMPEG_OLD__
...
{
    {CODEC_ID_MP3,            __FORMAT_AV_TY_A__,     1,	"mp3"}, //tested ok
    {CODEC_ID_AAC,            __FORMAT_AV_TY_A__,     0,	"aac"}, //tested ok
    {CODEC_ID_H264,           __FORMAT_AV_TY_V__,     0,	"h264"},  //tested ok
    {CODEC_ID_MPEG2VIDEO,     __FORMAT_AV_TY_V__,     0,	"mpeg2video"},
    {CODEC_ID_MPEG4,          __FORMAT_AV_TY_V__,     0,	"mpeg4"},
};
...
int __bb_dx_init_libavformat(void *fnp, int *ret_malloc_sz)
{
	...
	dmxp->fnp_dx_file_close         = dx_libavformat_file_read_close;
...
int dx_libavformat_load(void *r_data, NR_MEDIA *nmp)
{
    ...
    uuprintf("libffmpeg.so pointer = %p", dllsop);
    ...
    bbp->__xx_ffurl_register_protocol = (int (*)(URLProtocol *, int))dlsym(dllsop, "ffurl_register_protocol");
    if( bbp->__xx_ffurl_register_protocol == 0 ) return 1;
#ifdef __DP_FFMPEG_NEW__
    bbp->__xx_avformat_open_input = 0;
    bbp->__xx_avformat_open_input = (int (*)(AVFormatContext **, const char *, AVInputFormat *, AVDictionary **))dlsym(dllsop, "avformat_open_input");
    if( bbp->__xx_avformat_open_input == 0 ) return 1;
#endif
#ifdef __DP_FFMPEG_OLD__
    bbp->__xx_av_open_input_file =  0;
    bbp->__xx_av_open_input_file = (int (*)(AVFormatContext **, const char *, AVInputFormat *, int, AVFormatParameters *))dlsym(dllsop, "av_open_input_file");
    if( bbp->__xx_av_open_input_file == 0 ) return 1;
#endif
    bbp->__xx_av_close_input_file = 0;
    bbp->__xx_av_close_input_file = (void (*)(AVFormatContext *))dlsym(dllsop, "av_close_input_file");
    if( bbp->__xx_av_close_input_file == 0 ) return 1;

    bbp->__xx_av_find_stream_info = 0;
    bbp->__xx_av_find_stream_info = (int (*)(AVFormatContext *))dlsym(dllsop, "av_find_stream_info");
    if( bbp->__xx_av_find_stream_info == 0 ) return 1;

    bbp->__xx_av_read_frame = 0;
    bbp->__xx_av_read_frame = (int (*)(AVFormatContext *, AVPacket *))dlsym(dllsop, "av_read_frame");
    if( bbp->__xx_av_read_frame == 0 ) return 1;

    bbp->__xx_avformat_seek_file = 0;
    bbp->__xx_avformat_seek_file = (int (*)(AVFormatContext *, int, int64_t, int64_t, int64_t, int))dlsym(dllsop, "avformat_seek_file");
    ...
    bbp->__xx_avcodec_close = (int (*)(AVCodecContext *))dlsym(dllsop, "avcodec_close");
    if( bbp->__xx_avcodec_close == 0 ) return 1;

    bbp->__xx_av_free_packet = 0;
    bbp->__xx_av_free_packet = (void (*)(AVPacket *))dlsym(dllsop, "av_free_packet");
    ...
    bbp->__xx_av_dict_set = (int (*)(AVDictionary **, const char *, const char *, int))dlsym(dllsop, "av_dict_set");
    if( bbp->__xx_av_dict_set == 0 ) return 1;

    bbp->__xx_av_dict_free = 0;
    bbp->__xx_av_dict_free = (void (*)(AVDictionary **))dlsym(dllsop, "av_dict_free");
    if( bbp->__xx_av_dict_free == 0 ) return 1;
#endif
    bbp->__xx_av_malloc = 0;
    bbp->__xx_av_malloc = (void *(*)(size_t))dlsym(dllsop, "av_malloc");
    if( bbp->__xx_av_malloc == 0 ) return 1;

    bbp->__xx_av_free = 0;
    bbp->__xx_av_free = (void (*)(void *))dlsym(dllsop, "av_free");
...
extern URLProtocol isfd_protocol;
...
int dx_libavformat_file_read_open(void *r_data, char *filename, NR_MEDIA *nmp)
{
	...
	int64_t t_calc_duration_v=0, t_calc_duration_a=0;
	...
	(*(bbp->__xx_ffurl_register_protocol))(&isfd_protocol, sizeof(isfd_protocol));
#ifdef __DP_FFMPEG_NEW__
	//bbp->iformat = NULL;
	//bbp->dict = NULL;
...
uuprintf("find demuxer %d\n", tmp);
	...
	if( tmp == 0 )
	{
		fprintf(stdout,	"Can't find demuxer\n");
		fflush(stdout);
		exit(1);
	}
	...
	for( i=0 ; i<(int)(nmp->track_sz) ; i++ ) {
		...
		if( i == bbp->videoStream ) {
            ...
            ntp->ffmpeg_extra_size          = pCodecCtx->extradata_size;
            if( pCodecCtx->extradata_size > 0 )
            {
                ntp->ffmpeg_extra_data_ptr      = (uint64_t)malloc(pCodecCtx->extradata_size);
                memcpy((void *)(ntp->ffmpeg_extra_data_ptr), (void *)(pCodecCtx->extradata), pCodecCtx->extradata_size);
            }
...
uuprintf("__codec_match__ video %s\n", pCodec->name);
			nmp->video_track_enable = 0;
			for( j=0 ; j<(int)(sizeof(__codec_match__)/sizeof(__FORMAT_MATCH__)) ; j++ )	{
				if( strcmp(pCodec->name, __codec_match__[j].name) == 0 ) {
                    nmp->fourcc_ex2 = __codec_match__[j].i_ty;
                    if( __codec_match__[j].enable == 1 ) nmp->video_track_enable = 1;
					break;
				}
			}

			if( nmp->fourcc_ex2 == CODEC_ID_H264 )	{
		...
		if( i == bbp->audioStream )	{
            ...
            if( pCodecCtx->channels > 2 ) { pCodecCtx->request_channels = 2; channels = 2;}
            else { pCodecCtx->request_channels = 2; channels = pCodecCtx->channels; }
			...
			ntp->ffmpeg_codec_ptr           = (uint64_t)pCodec;
			ntp->ffmpeg_codec_id            = (uint64_t)pCodecCtx->codec_id;
            ntp->ffmpeg_codec_tag           = pCodecCtx->codec_tag;
            ntp->ffmpeg_stream_codec_tag    = pCodecCtx->stream_codec_tag;
			ntp->a_chs						= channels;
			ntp->a_req_chs					= pCodecCtx->request_channels;
...
uuprintf("__codec_match__ audio %s\n", pCodec->name);
			nmp->audio_track_enable = 0;
			for( j=0 ; j<(int)(sizeof(__codec_match__)/sizeof(__FORMAT_MATCH__)) ; j++ )	{
				if( strcmp(pCodec->name, __codec_match__[j].name) == 0 ) {
                    nmp->fourcc_ex3 = __codec_match__[j].i_ty;
                    if( __codec_match__[j].enable == 1 ) nmp->audio_track_enable = 1;
					break;
				}
			}
		}
...
uuprintf("<<<<<====== media duration %f", (float)(nmp->duration_ms)/1000);
...
// codec_unload
    for( i=0 ; i<(nmp->track_sz) ; i++ )
    {
		...
		if( ntp->ffmpeg_extra_data_ptr ) free((void *)(ntp->ffmpeg_extra_data_ptr));
    ...
    default:
		if( mode == 1 )	{
			...
			ntp->t_calc_duration = (int64_t)(avp->duration - avp->start_time) * 1000 * 1000
	                                                * avp->time_base.num
	                                                / avp->time_base.den;
			if( ntp->t_calc_duration <= 0 ) ntp->t_calc_duration = ((AVFormatContext *)(ntp->ffmpeg_avformat_ptr))->duration;
		} else if( mode == 2 ) {
...
if( track_no == 1 )
{
        uuprintf("dx_format track:%d,%d", track_no, nfp->data_size);
        uuprintf(" flags:%4d,", packet.flags);
        uuprintf(" pts:%10d,", packet.pts);
        uuprintf(" dts:%10d\n", packet.dts);
        __hexdump__("aac", (const void *)nfp->data_ptr, (size_t)nfp->data_size);
}
*/
...
uuprintf("dx_seek, %d:%f:%lld\n", track_no, (float)msec/1000, ts);
	if( (int64_t)msec > nmp->duration_ms ) return 1;
